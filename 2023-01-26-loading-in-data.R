...
b1 <- billboard %>% 
  pivot_longer(
    ...
    values_to = "Rank", # name of new column which will contain the values associated with the artist
    values_drop_na = TRUE # removes any rows where the value = NA
  )

View(b1) ## rmarkdown does NOT like this... remove to knit

### pivot_wider
### best for occupancy matrix

glimpse(fish_encounters)
View(fish_encounters)

fish_encounters %>% 
  pivot_wider(names_from = station, # which column you want to split into multiple columns
